...
- **Single Source of Truth (SSOT) for Client UI**: All UI decks, cards, HUD buttons, inventory displays, and shop items **MUST** dynamically read `SupplyCost`, `DisplayName`, `RoleTags`, and stats directly from the Shared Domain Catalog (`Catalog.Get(id)` / `Catalog.GetAll()`). **STRICTLY PROHIBITED** to duplicate or hardcode card cost tables (e.g. `CARD_COSTS = {...}`) in Client Presenters, Views, or Controllers.
